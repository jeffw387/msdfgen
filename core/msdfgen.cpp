...
static inline bool
pixelClash(const FloatRGB &a, const FloatRGB &b, double threshold) {
  ...
  // If the change is 0 <-> 1 or 2 <-> 3 channels and not 1 <-> 1 or 2 <-> 2, it
  // is not a clash
  if ((a.r > .5f && a.g > .5f && a.b > .5f) ||
      ...
      (b.r < .5f && b.g < .5f && b.b < .5f))
    return false;
  // Find which color is which: _a, _b = the changing channels, _c = the
  // remaining one
  ...
  return (fabsf(aa - ba) >= threshold) && (fabsf(ab - bb) >= threshold) &&
         fabsf(ac - .5f) >= fabsf(bc - .5f);  // Out of the pair, only flag the
                                              // pixel farther from a shape edge
}
  ...
  for (int y = 0; y < h; ++y)
    for (int x = 0; x < w; ++x) {
      if ((x > 0 && pixelClash(output(x, y), output(x - 1, y), threshold.x)) ||
          (x < w - 1 &&
           pixelClash(output(x, y), output(x + 1, y), threshold.x)) ||
          (y > 0 && pixelClash(output(x, y), output(x, y - 1), threshold.y)) ||
          (y < h - 1 &&
           pixelClash(output(x, y), output(x, y + 1), threshold.y)))
        clashes.push_back(std::make_pair(x, y));
    }
  for (std::vector<std::pair<int, int> >::const_iterator clash =
           clashes.begin();
       clash != clashes.end();
       ++clash) {
    ...
    for (int y = 0; y < h; ++y) {
      ...
      for (int x = 0; x < w; ++x) {
        ...
        for (int i = 0; i < contourCount; ++i, ++contour) {
          ...
          for (std::vector<EdgeHolder>::const_iterator edge =
                   contour->edges.begin();
               edge != contour->edges.end();
               ++edge) {
          ...
          if (windings[i] > 0 && minDistance.distance >= 0 &&
              fabs(minDistance.distance) < fabs(posDist))
            posDist = minDistance.distance;
          if (windings[i] < 0 && minDistance.distance <= 0 &&
              fabs(minDistance.distance) < fabs(negDist))
        ...
        if (posDist >= 0 && fabs(posDist) <= fabs(negDist)) {
          ...
          for (int i = 0; i < contourCount; ++i)
            if (windings[i] > 0 && contourSD[i] > sd &&
                fabs(contourSD[i]) < fabs(negDist))
              sd = contourSD[i];
    ...
    for (int y = 0; y < h; ++y) {
      ...
      for (int x = 0; x < w; ++x) {
        ...
        if (posDist >= 0 && fabs(posDist) <= fabs(negDist)) {
          ...
          winding = 1;
          for (int i = 0; i < contourCount; ++i)
            if (windings[i] > 0 && contourSD[i] > psd &&
                fabs(contourSD[i]) < fabs(negDist))
              psd = contourSD[i];
        } else if (negDist <= 0 && fabs(negDist) <= fabs(posDist)) {
          psd = negDist;
    ...
    for (int y = 0; y < h; ++y) {
      ...
      for (int x = 0; x < w; ++x) {
        ...
        for (int i = 0; i < contourCount; ++i, ++contour) {
          ...
          double medMinDistance = fabs(median(
              r.minDistance.distance,
              g.minDistance.distance,
              b.minDistance.distance));
          if (medMinDistance < d) {
          ...
          if (r.nearEdge)
            (*r.nearEdge)
                ->distanceToPseudoDistance(r.minDistance, p, r.nearParam);
          if (g.nearEdge)
            (*g.nearEdge)
                ->distanceToPseudoDistance(g.minDistance, p, g.nearParam);
          if (b.nearEdge)
            (*b.nearEdge)
                ->distanceToPseudoDistance(b.minDistance, p, b.nearParam);
          medMinDistance = median(
              r.minDistance.distance,
              g.minDistance.distance,
              b.minDistance.distance);
          ...
          if (windings[i] > 0 && medMinDistance >= 0 &&
              fabs(medMinDistance) < fabs(posDist))
            posDist = medMinDistance;
          if (windings[i] < 0 && medMinDistance <= 0 &&
              fabs(medMinDistance) < fabs(negDist))
            negDist = medMinDistance;
        }
        if (sr.nearEdge)
          (*sr.nearEdge)
              ->distanceToPseudoDistance(sr.minDistance, p, sr.nearParam);
        if (sg.nearEdge)
          (*sg.nearEdge)
              ->distanceToPseudoDistance(sg.minDistance, p, sg.nearParam);
        if (sb.nearEdge)
          (*sb.nearEdge)
        ...
        if (posDist >= 0 && fabs(posDist) <= fabs(negDist)) {
          ...
          for (int i = 0; i < contourCount; ++i)
            if (windings[i] < 0 && contourSD[i].med < msd.med &&
                fabs(contourSD[i].med) < fabs(posDist))
              msd = contourSD[i];
        }
        for (int i = 0; i < contourCount; ++i)
...
#pragma omp parallel for
  ...
  for (int y = 0; y < h; ++y) {
    ...
    for (int x = 0; x < w; ++x) {
      ...
      Point2 p = Vector2(x + .5, y + .5) / scale - translate;
      SignedDistance minDistance;
      for (std::vector<Contour>::const_iterator contour =
